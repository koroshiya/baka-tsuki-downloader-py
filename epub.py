...
def addOPFMeta(version, title, uuid, cover, coverimg, author, cList, fullImage, nps):
...
def addToc(version, nps, css, uid):
	...
	htmlstr = cleanET(html, True, True)
...
def addTitlePage(title, css):
	...
	return cleanET(html, True)

def cleanET(html, convert=False, leaveHref=False):
...
def parseNode(inx, tex, content, tag, css, zf, doubleImages):
...
#bURL = "http://www.baka-tsuki.org/project/index.php?title=Log_Horizon:_Volume_7" #TODO: Turn into prompt
...
zf = zipfile.ZipFile(title + '.epub', mode='w')
...
for li in toc.findall("ul/li"):
	...
	zf.writestr('OEBPS/content/'+num+'.html', parseNode(curH2, tex, content, 'h2', 'stylesheet.css', zf, doubleImages))
	...
	for uan in uans:
		...
		zf.writestr('OEBPS/content/'+num+'_'+unum+'.html', parseNode(curH3, utex, content, 'h3', 'stylesheet.css', zf, doubleImages))
...
zf.writestr('META-INF/container.xml', addContainer())
zf.writestr('OEBPS/' + tocFile, addToc(version, tocList, 'content/stylesheet.css', bURL))
zf.write('stylesheet.css', arcname='OEBPS/content/stylesheet.css')
zf.writestr('OEBPS/title.html', addTitlePage(title, 'content/stylesheet.css'))
zf.writestr('OEBPS/content.opf', addOPFMeta(version, title, bURL, 'title.html', coverImg, author, cList, fullImage, tocList))
